...
export default function VaporizeTextCycle({
  ...
  const baseFontSize = parseInt(font.fontSize?.replace("px", "") || "50");
  const responsiveFontSize = wrapperSize.width ? Math.min(baseFontSize, Math.max(32, wrapperSize.width / 8.5)) : baseFontSize;
  ...
  const fontConfig = useMemo(() => {
    const VAPORIZE_SPREAD = calculateVaporizeSpread(responsiveFontSize);
    const MULTIPLIED_VAPORIZE_SPREAD = VAPORIZE_SPREAD * spread;
    return {
      fontSize: responsiveFontSize,
      VAPORIZE_SPREAD,
      MULTIPLIED_VAPORIZE_SPREAD,
      font: `${font.fontWeight ?? 400} ${responsiveFontSize * globalDpr}px ${font.fontFamily}`,
    };
  }, [responsiveFontSize, font.fontWeight, font.fontFamily, spread, globalDpr]);
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      // Performance Optimization: If static or waiting, render once and pause loop
...
const renderCanvas = ({
  ...
  // Parse font size and make it responsive to canvas width
  const baseFontSize = parseInt(framerProps.font?.fontSize?.replace("px", "") || "50");
  const responsiveFontSize = Math.min(baseFontSize, Math.max(32, width / 8.5));
  const font = `${framerProps.font?.fontWeight ?? 400} ${responsiveFontSize * globalDpr}px ${framerProps.font?.fontFamily ?? "sans-serif"}`;
...
const createParticles = (
  ...
  if (text === "PreciseHire") {
    ...
    // Set align to left for drawing segments sequentially
    ctx.textAlign = "left";
    
    // Draw part 1 (Precise) in deep slate
    ctx.fillStyle = "#151e2e";
    ctx.fillText(part1, textBoundaries.left, textY);
    
    // Draw part 2 (Hire) in primary blue
    ctx.fillStyle = "#2563eb";
    ctx.fillText(part2, textBoundaries.left + part1Width, textY);
  } else {
    ctx.fillStyle = color;
